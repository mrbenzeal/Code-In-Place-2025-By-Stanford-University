...
"""
# The rhoomba karel example from the video!
...
def main():
    """
    Clear the world, row by row. 
    Each time a row is cleared, 
    reset to the start of the row to create
	...
	# Note: the while-loop always assumes that Karel is facing right (east)
        clear_row()
        reset_to_next_row()
    # fencepost problem! One more row to clear. This last clear_row() fixes it.
    clear_row()
...
def reset_to_next_row():
    """
    Make Karel moves from the end of the first row to the beginning of the second row.
